    ...
    def get_featured_publications(self):
        ...
        if len(featured_publications) == 0:
            ...
            featured_publications = PublicationPage.objects.prefetch_related(
                'authors__author',
                'topics',
            ).live().public().exclude(id__in=featured_page_ids).exclude(publication_type__title__in=['Working Paper', 'DigiFin Policy Brief', 'Unofficial Publications']).order_by('-publishing_date')[:4]
