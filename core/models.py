...
class ContentPage(Page, SearchablePageAbstract):
    ...
    def recommended_content(self):
        ...
        for topic in self.topics.all():
            topic_content.append(list(ContentPage.objects.live().public().filter(topics=topic, publishing_date__isnull=False).exclude(id=self.id).order_by('-publishing_date')))
        for i in range(10):
