...
def metric_pages(request):
    content_pages = ContentPage.objects.live().filter(multimediaseriespage=None).filter(publishing_date__range=["2016-01-01", "2021-10-31"])
    ...
    for content_page in content_pages:
        ...
        json_items.append({
            'id': str(content_page.id),
            'title': content_page.title if content_page.title else '',
            'subtitle': subtitle,
            'authors': authors if authors else '',
            'speakers': speakers if speakers else '',
            'published_date': publishing_date,
